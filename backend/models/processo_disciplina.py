...
class ProcessoDisciplina(db.Model):
    ...
    def prazo_ciencia(self):
        ...
        return base_date + timedelta(hours=24) if base_date else None

    @property
    def prazo_defesa(self):
        if self.data_ciente:
            return self.data_ciente + timedelta(hours=24)
        if self.status == StatusProcesso.ALUNO_NOTIFICADO.value:
            base = self.data_registro or self.data_ocorrencia
    ...
    def prazo_recurso(self):
        ...
        if self.status == StatusProcesso.DECISAO_EMITIDA.value and self.data_decisao:
