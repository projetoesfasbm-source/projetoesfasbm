...
class UserService:
    ...
    def get_current_school_id():
        """
        Retorna a escola atual. Confia na sessão, pois ela foi definida no login.
        """
        ...
        if current_user.role in ['super_admin', 'programador']:
            ...
            return int(view_as) if view_as else None

        # 2. Confiança na Sessão (Setada na tela de seleção)
        active_id = session.get('active_school_id')
        if active_id:
            try:
                # Opcional: Validar se ainda pertence à lista (Memória rápida)
                for us in getattr(current_user, 'user_schools', []):
                    if us.school_id == int(active_id):
                        return int(active_id)
            except:
                pass

        # 3. Fallback (Se algo deu muito errado ou tem apenas 1 escola)
        first_school = db.session.execute(
            select(UserSchool).filter_by(user_id=current_user.id).order_by(UserSchool.school_id)
        ).scalars().first()

        if first_school:
            session['active_school_id'] = first_school.school_id
            return first_school.school_id
