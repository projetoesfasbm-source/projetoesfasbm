...
class JusticaService:
    ...
    def _is_curso_pontuado(aluno_id=None, school=None, turma=None):
        try:
            ...
            # Apenas CBFPM e CSPM pontuam. CTSP retorna False.
    ...
    def calcular_ndisc_aluno(aluno_id):
        ...
        if not dt_corte: return JusticaService.NDISC_BASE 
    ...
    def calcular_fada_estimada(aluno_id):
        ...
        dt_corte = JusticaService.get_data_inicio_2_ciclo(aluno.turma_id)
        if not dt_corte: dt_corte = datetime.min.astimezone()
        ...
        descontos = 0.0
        for p in processos:
            if p.is_crime: descontos += JusticaService.DESC_FADA_CRIME
            elif p.origem_punicao == 'RDBM': descontos += JusticaService.DESC_FADA_RDBM
            else:
        ...
        query_elogios = select(Elogio).where(Elogio.aluno_id == aluno_id, Elogio.data_elogio >= dt_corte)
    ...
    def calcular_aat_final(aluno_id):
        if not JusticaService._is_curso_pontuado(aluno_id=aluno_id):
        ...
        if fada_oficial:
            nota_fada = fada_oficial.media_final
        else:
            nota_fada = JusticaService.calcular_fada_estimada(aluno_id)
    ...
    def criar_processo(descricao, observacao, aluno_id, autor_id, pontos=0.0, codigo_infracao=None, regra_id=None, data_ocorrencia=None):
        try:
            # BLINDAGEM CTSP: Zera pontos sempre
            if not JusticaService._is_curso_pontuado(aluno_id=aluno_id):
                pontos = 0.0
