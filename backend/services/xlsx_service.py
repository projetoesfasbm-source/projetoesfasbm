...
def gerar_mapa_gratificacao_xlsx(
    ...
    auxiliar_funcao: Optional[str] = None,      # <<< NOVO PARÂMETRO DE FUNÇÃO
    comandante_funcao: Optional[str] = None,    # <<< NOVO PARÂMETRO DE FUNÇÃO
    *,
    ...
    # Cabeçalho da tabela
    headers = ["Posto / graduação", "Matrícula", "Nome completo do servidor", "Disciplina", "CH total", "CH paga anteriormente no acumulado dos mapas", "CH a pagar", "Valor em R$ CH a Pagar"]
    start_table_row = r
    ...
    for instrutor in (dados or []):
        ...
        for disc in _iter_disciplinas(instrutor):
            row_vals = [
                ...
                float(int(_safe(disc, "ch_a_pagar", 0) or 0)) * float(valor_hora_aula or 0)
            ]
            for c_offset, v in enumerate(row_vals):
                c = main_start_col + c_offset
                cell = ws.cell(r, c, v)
                cell.border = border_all
                if c_offset in (0, 1, 4, 5, 6): cell.alignment = center
                elif c_offset in (2, 3): cell.alignment = Alignment(horizontal="left", vertical="center")
                elif c_offset == 7:
                    cell.style = brl
    ...
    c_tot_label = ws.cell(r, main_start_col, "CARGA HORARIA"); c_tot_label.fill = gray; c_tot_label.font = total_font; c_tot_label.alignment = right
    c_tot_ch = ws.cell(r, main_end_col - 1, total_ch_a_pagar); c_tot_ch.alignment = center; c_tot_ch.font = total_font; c_tot_ch.fill = gray
    c_tot_vl = ws.cell(r, main_end_col, total_valor); c_tot_vl.style = brl; c_tot_vl.alignment = right; c_tot_vl.font = total_font; c_tot_vl.fill = gray
    for i in range(main_start_col, main_end_col + 1): ws.cell(r, i).border = border_all
    r += 1
    ...
    ws.merge_cells(start_row=left_start_row, start_column=1, end_row=left_end_row, end_column=8); c_left = ws.cell(left_start_row, 1, left_text); c_left.alignment = left_top
    ws.merge_cells(start_row=left_start_row, start_column=9, end_row=left_end_row, end_column=14); c_right = ws.cell(left_start_row, 9, right_text); c_right.alignment = center_top
    ...
    # -------------------------
    # Linha de assinatura
    ws.merge_cells(start_row=r, start_column=4, end_row=r, end_column=6); ws.cell(r, 4, "____________________________________").alignment = center
    ws.merge_cells(start_row=r, start_column=8, end_row=r, end_column=10); ws.cell(r, 8, "____________________________________").alignment = center
    r += 1
    # Linha com os Nomes
    ws.merge_cells(start_row=r, start_column=4, end_row=r, end_column=6); ws.cell(r, 4, (auxiliar_nome or "")).alignment = center
    ws.merge_cells(start_row=r, start_column=8, end_row=r, end_column=10); ws.cell(r, 8, (comandante_nome or "")).alignment = center
    r += 1
    # <<< SEÇÃO CORRIGIDA PARA USAR PARÂMETROS DE FUNÇÃO >>>
