...
def registrar_em_massa():
    """
    Registra infrações ou elogios para múltiplos alunos.
    Trata campos específicos para CRIME e RDBM.
    """
    tipo = request.form.get('tipo_registro') # 'infracao' ou 'elogio'
    ...
    if tipo == 'infracao':
        ...
        observacao = request.form.get('observacao') # Termo de justificação
        
        # Novos Campos do Formulário Atualizado
        origem_punicao = request.form.get('origem_punicao', 'NPCCAL') # NPCCAL, RDBM
        is_crime_str = request.form.get('is_crime', 'false')
        is_crime = (is_crime_str.lower() == 'true')
        codigo_infracao_manual = request.form.get('codigo_infracao') # Para RDBM/Crime

        # Define pontos iniciais (serão recalculados no final se necessário, mas bom ter referência)
        pontos_iniciais = 0.0
        ...
        for aid in alunos_ids:
            # Criação via Service (que define status inicial)
            # Nota: O Service create_processo básico não aceita todos os campos extras ainda, 
            # então instanciamos manualmente aqui para garantir Crime/RDBM corretos.
            try:
        ...
        for aid in alunos_ids:
            try:
                novo_elogio = Elogio(
                    aluno_id=int(aid),
                    registrado_por_id=current_user.id, # IMPORTANTE: Campo corrigido conforme erro anterior
                    data_elogio=data_completa,
                    descricao=descricao,
                    pontos=0.5 # Valor padrão (Art 125)
                )
...
@login_required
def fada_boletim():
    """Lista alunos para avaliação FADA."""
    school_id = UserService.get_current_school_id()
    if not school_id: return redirect(url_for('main.dashboard'))
    
    # Busca alunos com suas turmas
    stmt = select(Aluno).join(Turma).where(Turma.school_id == school_id).order_by(Turma.nome, Aluno.nome_completo)
    ...
    return render_template('justica/fada_lista_alunos.html', alunos=lista)

@justica_bp.route('/api/infracoes-pendentes/<int:aluno_id>')
@login_required
def api_infracoes_pendentes(aluno_id):
    """
    API JSON para o formulário FADA.
    Retorna infrações (para desconto) e elogios (para bônus).
    """
    ...
    for e in elogios:
        resultado.append({
            ...
            'pontos': 0.50, # Valor padrão do elogio
            'tipo': 'elogio'
        })
...
@can_manage_justice_required
def salvar_fada():
    """Salva a avaliação manual final."""
    ...
    try:
        ...
        nova_fada = FadaAvaliacao(
            ...
            # Aqui você poderia salvar as notas individuais se o modelo FadaAvaliacao tiver campos json ou colunas
            # Se o modelo tiver colunas nota_1...nota_18, teria que mapear aqui.
            # Assumindo modelo simples com média:
        )
        
        # Se seu modelo FadaAvaliacao tem colunas explicitas (criterio_1, criterio_2...), mapeie aqui:
        # for i, nota in enumerate(notas):
        #     setattr(nova_fada, f"criterio_{i+1}", float(nota))
...
# --- UTILITÁRIOS: APIs de Alunos e PDF ---

@justica_bp.route('/api/alunos-por-turma/<int:turma_id>')
@login_required
def get_alunos_turma(turma_id):
    alunos = db.session.scalars(select(Aluno).where(Aluno.turma_id == turma_id)).all()
    data = []
    for a in alunos:
        data.append({
            'id': a.id,
...
def exportar_selecao():
    # Renderiza página para seleção de exportação
