...
# Importa o decorator correto e restritivo
...
@vinculo_bp.route('/')
@login_required
@sens_or_admin_or_programmer_required
def gerenciar_vinculos():
    ...
    # O decorator já garantiu que é SENS/Admin/Prog. Não precisa de if manual.
    ...
    # can_manage_vinculos=True pois o decorator garantiu o acesso
...
def adicionar_vinculo():
    ...
    if request.method == 'POST':
        ...
        # Validação segura por ID e School ID
...
def editar_vinculo(vinculo_id):
    ...
    # --- CORREÇÃO DE VALIDAÇÃO (SEM STRINGS) ---
    # Busca a disciplina e, através dela, a turma.
    if not vinculo.disciplina:
        flash("Erro de integridade: Vínculo sem disciplina associada.", "danger")
        return redirect(url_for('vinculo.gerenciar_vinculos'))

    # Pega a turma real pelo ID relacional, ignorando o nome 'pelotao'
    turma_real = vinculo.disciplina.turma

    # Valida se a turma existe e pertence à escola atual
    ...
    # Carrega as disciplinas da turma correta (validada por ID acima)
    ...
    if form.validate_on_submit():
        ...
        flash(message, 'success' if success else 'danger')
        # Redireciona filtrando pela turma recém editada
...
def excluir_vinculo(vinculo_id):
    ...
    # --- CORREÇÃO DE VALIDAÇÃO (MESMA LÓGICA DO EDITAR) ---
    if not vinculo.disciplina:
        flash("Erro de integridade: Vínculo sem disciplina associada.", "danger")
        return redirect(url_for('vinculo.gerenciar_vinculos'))
    ...
    turma_id_para_redirecionar = turma_real.id
